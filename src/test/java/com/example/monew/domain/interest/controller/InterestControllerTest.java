...
class InterestControllerTest {
  ...
  @Test
  @DisplayName("POST /api/interests: 유효 요청 → 201 + 응답 바디 (헤더 제거)")
  void create201() throws Exception {
    ...
    verify(interestService).create(any(InterestCreateRequest.class));
  }
  ...
  @Test
  @DisplayName("PATCH /api/interests/{interestId}: name 포함 → 400 INVALID_REQUEST")
  ...
  @Test
  @DisplayName("GET /api/interests: 성공 → 200 + 서비스에 userId null 전달")
  void list200() throws Exception {
    ...
    when(interestService.getInterests(any(), any(), any(), any(), any(), anyInt(), eq(null)))
    ...
    verify(interestService).getInterests(any(), any(), any(), any(), any(), anyInt(), eq(null));
  }
  ...
  void listInvalidSort400() throws Exception {
    when(interestService.getInterests(any(), eq("foo"), any(), any(), any(), anyInt(), eq(null)))
