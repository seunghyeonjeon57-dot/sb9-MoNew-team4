...
public class GlobalExceptionHandler {
  ...
  // 쿼리 파라미터 / PathVariable 타입 변환 실패 (예: UUID 자리에 문자열)
  @ExceptionHandler(MethodArgumentTypeMismatchException.class)
  public ResponseEntity<ErrorResponse> handleTypeMismatch(MethodArgumentTypeMismatchException e) {
    Map<String, Object> details = new HashMap<>();
    details.put("parameter", e.getName());
    details.put("value", String.valueOf(e.getValue()));
    log.warn("파라미터 타입 변환 실패: parameter={}, value={}", e.getName(), e.getValue());
    return respond(ErrorCode.INVALID_REQUEST, e, details);
  }
